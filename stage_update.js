(function(){
  ...
  function 是否其他项目(p){return typeof isActiveProject==='function'?!isActiveProject(p):/已完成|已完工|完工|完结|已完结|结束|已结束|结案|已结案|归档|已归档|暂停|已暂停|停滞|取消|已取消|作废|关闭/.test([p&&p.stage,p&&p.status,p&&p.project_status,p&&p.state,p&&p.raw&&p.raw.status,p&&p.raw&&p.raw.stage,p&&p.raw&&p.raw.project_status,p&&p.raw&&p.raw.state].map(取文本).join(' '))}
  ...
  function 应用阶段(){
    ...
    (d.projects||[]).forEach(function(p){
      ...
      if(v){
        ...
        p.isCompleted=typeof isActiveProject==='function'?!isActiveProject(p):(v==='已完工'||v==='暂停'||v==='取消');
      }
      项目表[String(p.id)]=p;
    });
